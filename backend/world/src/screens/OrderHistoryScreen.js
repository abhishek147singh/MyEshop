import { Paper ,CircularProgress , Alert , Button, Table, TableBody, TableCell, TableHead, TableRow, TableContainer } from '@mui/material'
            ...
            <h1>Order history</h1>
            {
                (loading) ? (<div className='center'> <CircularProgress /> </div>)
                    : error ? (<Alert> {error} </Alert>)
                        : (
                            <Container component={Paper} style={{padding:"10px" , margin : "10px auto"}} >
                                ...
                                <TableBody>
                                    {
                                        orders.map((order) => (
                                            <StyledTableRow key = {order._id}>
                                                ...
                                                <StyledTableCell>{order.isDeliveredAt ? order.deliveredAt.substring(0 , 10) : 'No' }</StyledTableCell>
                                                <StyledTableCell><Button type='button' variant='outlined' onClick={() => navigate(`/order/${order._id}`)}>Details</Button></StyledTableCell>
                                            </StyledTableRow>
                                        ))
                                    }
                                </TableBody>
